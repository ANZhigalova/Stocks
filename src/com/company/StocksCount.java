...
//            }
    ...
    public static double gamma(double T, double r, int N, int n, double a, double b, double S)
    {
        ...
        double result = pow(1 + r, -(N - n)) *
                (Fn(S* (1 + b), a, b, N-n, T, p) - Fn(S * (1 + a), a, b, N-n, T, p)) / (S * (b - a));
        return result;
    }
    ...
    public static double betta(double B_0, double B_N, double T, double r, int N, int n, double a, double b, double S)
    {
        ...
        double result = Fn(S, a, b, N-(n-1), T, p) / B_N - pow(1 + r, -(N - n)) *
                ( Fn(S * (1 + b), a, b, N - n, T, p) - Fn(S * (1 + a) , a, b, N-n, T, p) ) / (B_0 * (b - a));
        return result;
    }
    ...
    public static double Fn(double x, double a, double b, int N, double T, double p)
    {
        ...
        for (int i = 0; i < N + 1; i++)
        {
            ...
            C_n_i = (double) factorial(N)/(factorial(i) * factorial(N - i));
            sum += max_0 * C_n_i * pow(p, i) * pow(1 - p, N - i);
        }
        return sum;
    }
